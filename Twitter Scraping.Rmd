...
follower_list <- read.csv("file_name.csv") #Just replace file_name with the file. You need the .csv, or it won't work.
```
...
write_excel_csv(user_timelines, file="good_name_here.csv") #Just put in what you want the file to be called. Don't forget the .csv at the end! It won't work without it. It should just stick the new file into same folder as this R Studio document.  
```
